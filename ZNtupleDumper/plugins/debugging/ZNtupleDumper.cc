...
private:
	...
	virtual void endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&);
        ...
        virtual Int_t getRingIndex(Int_t _x, Int_t _y, Int_t _z);
        virtual void Init_Map(std::string _map_file_path);
	// ----------member data ---------------------------
private:
...
private:
	...
	edm::Timestamp _eventTimeStamp;
        ...
        std::map<std::tuple<int, int, int>, int> ring_map;
...
private:
  ...
  ///SJ
	...
	Short_t _ySeedSC[NELE]                    = initInt;    ///< iphi(iy) of the SC seed in EB(EE)
        ...
        Int_t  _eeRingNoSeedSC[NELE]              = initInt;
	...
	Bool_t	  _ZEvent = false;
        ...
        ///added later SJ
  Float_t _ele1E, _ele2E, _ele1ecalE, _ele2ecalE, _angleEle12;
...
void ZNtupleDumper::beginJob()
{
	...
	///initializing the ring no. function
...
//					if(!eleIter2->parentSuperCluster().isNonnull()) continue;
	...
	if (_eventType == ZSC || _eventType == UNKNOWN || _eventType == MINIAOD) {
	  ...
	  //std::cout<<""<<std::endl;
	  //std::cout<<"SJ!!Inside ZSC skim"<<std::endl;
...
//	_tree->Branch("isMustacheSC", _isMustacheSC, "isMustacheSC[3]/b");
	...
	_tree->Branch("eeRingNoSeedSC",            _eeRingNoSeedSC,            "eeRingNoSeedSC[3]/I");
	...
	_tree->Branch("energyMCEle",	_energyMCEle,	"energyMCEle[3]/F");
	...
	///eleE, eleECALE
	_tree->Branch("ele1E",    &_ele1E,      "ele1E/F");
	_tree->Branch("ele2E",    &_ele2E,      "ele2E/F");
	_tree->Branch("ele1ecalE",    &_ele1ecalE,      "ele1ecalE/F");
	_tree->Branch("ele2ecalE",    &_ele2ecalE,      "ele2ecalE/F");
	_tree->Branch("angleEle12",    &_angleEle12,      "angleEle12/F");
...
void ZNtupleDumper::ResetMainTreeVar()
{
	...
	_ele1E = initSingleFloat;
	_ele2E = initSingleFloat;

	_ele1ecalE = initSingleFloat;
	_ele2ecalE = initSingleFloat;
	_angleEle12 = initSingleFloat;
}
...
{
  std::cout<<"Inside TreeSetSingleSCVar"<<std::endl;
	_etaSCEle[index] = sc.eta();
	std::cout<<"taking the phi now"<<std::endl;
	_phiSCEle[index] = sc.phi();
	std::cout<<"taking the nhits now"<<std::endl;
	_nHitsSCEle[index] = sc.size();

	std::cout<<"taking the raw wnegy"<<std::endl;
	_rawEnergySCEle[index]	   = sc.rawEnergy();
	//SJ
	std::cout<<"taking corrected energy "<<std::endl;
	_mustEnergySCEle[index]	   = sc.correctedEnergy();
	std::cout<<"took  corrected energy, which is "<<_mustEnergySCEle[index]<<std::endl;
	...
	if(seedDetId.subdetId() == EcalBarrel) {
		...
		_eeRingNoSeedSC[index] = getRingIndex(seedDetIdEcal.ix(),seedDetIdEcal.iy(), zSide);
	}
	...
	std::cout<<"before icalMap "<<std::endl;
	...
	std::cout<<"before LaserDB "<<std::endl;
	...
	std::cout<<"before hits and fractions "<<std::endl;
	std::vector< std::pair<DetId, float> > hitsAndFractions = sc.hitsAndFractions();
	for (const auto&  detitr : hitsAndFractions) {

	  std::cout<<"inside hits and fraction"<<std::endl;
	  EcalRecHitCollection::const_iterator oneHit = recHits->find( (detitr.first) ) ;
	  std::cout<<"oneHits "<<std::endl;
#ifdef DEBUG
	  assert(oneHit != recHits->end()); // DEBUG
#endif
	  std::cout<<"laserHandle "<<std::endl;
	  std::cout<<" detitr.first.rawID "<<detitr.first.rawId()<<std::endl;
	  std::cout<<" detitr.second "<<detitr.second<<std::endl;
	  sumLC_E += laserHandle->getLaserCorrection(detitr.first, _eventTimeStamp) * oneHit->energy();
	  //sumLC_E += laserHandle->getLaserCorrection(detitr.first, _eventTimeStamp);
	  std::cout<<"sumLC_E"<<std::endl;
	  sumE    += oneHit->energy();
...
void ZNtupleDumper:: TreeSetDiElectronVar(const pat::Electron& electron1, const pat::Electron& electron2)
{
	...
	_ele1E = _energyEle[0];
	_ele2E = _energyEle[1];
	_ele1ecalE = _energy_ECAL_ele[0];
	_ele2ecalE = _energy_ECAL_ele[1];
	_angleEle12 = angle;
...
void ZNtupleDumper::TreeSetDiElectronVar(const pat::Electron& electron1, const reco::SuperCluster& electron2)
{
  ...
  TreeSetSingleElectronVar(electron2, 1);
	...
	_ele1E = electron1.energy();
	_ele2E = electron2.energy();
	_ele1ecalE = _energy_ECAL_ele[0];
	_ele2ecalE = _energy_ECAL_ele[1];
	_angleEle12 = angle;
...
//			_ZRecHitSCEle[index].push_back(recHitId.zside());
			_ZRecHitSCEle[index].push_back(0);
		} else {
...
void ZNtupleDumper::Init_Map(std::string _map_file_path){
	//load map file
	...
	while (std::getline(read_map, line)){
		...
		std::vector<std::string> split_coordinate_string;
...
Int_t ZNtupleDumper::getRingIndex(Int_t _x, Int_t _y, Int_t _z){
  std::tuple<int, int, int> _coordinates((int)_x, (int)_y, (int)_z);
	if(ring_map.find(_coordinates) == ring_map.end()){
		// std::cout<<"\tRing index not found for coordinates ("<<_x<<", "<<_y<<", "<<_z<<") !" <<std::endl;
		return -999;
	};
	Int_t _ringIndex = ring_map[_coordinates];
	return _ringIndex;
};
