...
jsonPrompt=/afs/cern.ch/cms/CAF/CMSCOMM/COMM_DQM/certification/Collisions17/13TeV/PromptReco/Cert_294927-306462_13TeV_PromptReco_Collisions17_JSON.txt
jsonNamePrompt=294927-306462_ReReco_Nov2017_v1

scheduler=caf
#tag_Prompt=config/reRecoTags/92X_dataRun2_Prompt_v11.py
#tag_Prompt=config/reRecoTags/94X_dataRun2_ReReco_EOY17_v2.py
...
#tag_MC=config/reRecoTags/94X_mc2017_realistic_v10.py
#tag_MC=config/reRecoTags/80X_mcRun2_asymptotic_2016_TrancheIV_v6.py
tag_MC=config/reRecoTags/80X_mcRun2_asymptotic_2016_TrancheIV_v7.py

fileList=alcareco_datasets.dat
#PERIOD=RUN2017NEW_DCS
#PERIOD=RUN2017Scale
#PERIOD=RUN2017MCScale
#PERIOD=LEGACY2016
#PERIOD=MORIOND2017A
#PERIOD=MORIOND17 # MC
PERIOD=MORIOND17Scale # MC
...
extraName="Moriond17_Scale_ID80X"
...
do
	...
	case $datasetName in
		...
		*07Aug17*)
			...
			;;
#		*DYJets_madgraph*) #MC
#			./scripts/prodNtuples.sh --type=MINIAOD --isMC -t ${tag_MC} -s noSkim --scheduler=${scheduler} --extraName=${extraName} --doEleIDTree --check $dataset
#			;;
		*amcatnlo-RunIISummer16-miniAODv2*)
			./scripts/prodNtuples.sh --type=MINIAOD --isMC -t ${tag_MC} -s noSkim --scheduler=${scheduler} --extraName=${extraName} --doEleIDTree --check $dataset
			;;
		*miniAOD) #data prompt 2017
			...
			./scripts/prodNtuples.sh --type=MINIAOD -t ${tag_Prompt} -s noSkim --scheduler=${scheduler} --extraName=${extraName}   --json=$json --json_name=$jsonName   ${CHECK} $dataset
			;;
	esac
