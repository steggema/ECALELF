...
#    echo "=========================================================================================="
	...
	####2018 has change in the dataset names
	#if [ -z "${SINGLEELE}" -a "`echo $dataset | grep -c EcalUncalWElectron`" != "0" ];then 
	if [ -z "${SINGLEELE}" -a "`echo $dataset | grep -c WElectron`" != "0" ];then 
	    echo "this is W dataset with SINGLEELE not set so CONTINUE!!!" 
	    continue; 
	fi
	#if [ "${SINGLEELE}" -a "`echo $dataset | grep -c EcalUncalZElectron`" != "0" ];then 
	if [ "${SINGLEELE}" -a "`echo $dataset | grep -c ZElectron`" != "0" ];then 
	    echo "this is Z dataset with SINGLEELE SET so CONTINUE!!!" 
	    continue; 
	fi
...
#	echo " [INFO] Dataset $dataset"
	...
	if [ -n "${RERECO}" ];then
		...
		./scripts/prodNtuples.sh  -t ${TAGFILE} --type=ALCARERECO \
  			${JSONFILE} ${JSONNAME} \
			--scheduler=$SCHEDULER --file_per_job=10 --isMC  --doEleIDTree --doExtraStudyTree --doExtraCalibTree --extraName=pedNoise  ${DOEXTRACALIBTREE} ${EXTRAOPTION} ${EXTRAEXTRAOPTION} \
			${TUTORIAL} $dataset 
