...
runMin=`dasgoclient --query="run dataset=$DATASETPATH | min(run.run_number)" | tail -1 | cut -d '=' -f 2`
runMax=`dasgoclient --query="run dataset=$DATASETPATH | max(run.run_number)" | tail -1 | cut -d '=' -f 2`
...
#dasgoclient --query="run dataset=$DATASETPATH |sort run.modification_time" --format=json --limit=1000 | sed 's|"run":|"\nrun":|g' | grep modification_time >  das.json
...
#dasgoclient --query="run dataset=$DATASETPATH run.modification_time=

dasgoclient --query="config dataset=${DATASETPATH} | grep config.global_tag" | sed '/Showing/ d'
./scripts/lastJSON.sh
