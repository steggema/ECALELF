...
if [ "`cat $UI_WORKING_DIR/${FILENAME_BASE}.list | wc -l | awk '{print $1}' `" != "1" ];then
...
###SJ
if [ "`cat $UI_WORKING_DIR/${FILENAME_BASE}.list | wc -l | awk '{print $1}' `" == "1" ];then
    tmpDir=/tmp/$USER/$UI_WORKING_DIR/
    mkdir -p $tmpDir
    echo "only 1 ntuple is present so just copying it outside!!!"
    fname=`cat $UI_WORKING_DIR/${FILENAME_BASE}.list`
    
    hadd /tmp/$USER/$UI_WORKING_DIR/${MERGEDFILE} $fname
fi
